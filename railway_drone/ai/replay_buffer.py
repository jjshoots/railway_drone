    ...
    def push(self, data):
        if self.counter == 0:
            ...
            for i in range(len(data)):
                self.memory.append(np.zeros((self.mem_size, *data[i].shape), dtype=np.float32))

        assert len(data) == len(self.memory), 'data length not similar to memory buffer length'
