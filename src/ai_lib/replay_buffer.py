...
class ReplayBuffer():
    ...
    def push(self, state, next_state, action, next_action, reward, done, info):
        if self.counter == 0:
            ...
            self.info = np.zeros((self.mem_size, *info.shape), dtype=np.float32)
        ...
        self.info[i] = info.detach().cpu().numpy()
    ...
    def pull(self, batch_size, device='cuda:0'):
        ...
        info = torch.tensor(np.take(self.info, idx, 0)).to(device)

        return state, next_state, action, next_action, reward, done, info
